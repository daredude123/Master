    ...
    foreach my $y ($wn->querySense($x,"syns")){
      ...
      if(scalar (split(/ /,$y)) > 0){
        for(my $i =0; $i < scalar @HeadLineArray; $i++){
          ...
          #Below is the multiplication lines: If the word is equal to the sense of the word and not the synset it gets a 2 times multiplier and 1.5 if it is equal to the a senses accompanying synset.
      ...
      foreach my $z (ReturnSenses->returnPOSSenses($glossW,(lc substr $glossWPOS,0,1))) {
        ...
        foreach my $xy (@cont) {
          ...
          foreach my $xyz (ReturnSenses->returnSenses($contextW)) {
            ...
            $subValueHash{$x} += $measure->getRelatedness($xyz,$z);
